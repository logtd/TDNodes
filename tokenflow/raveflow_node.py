...
import comfy.conds
from ..rave.rave_sampling_config import RaveSamplingConfig
from ..rave import rave_utils as ru
...
from ..rave.rave_controlnet_utils import prepare_controlnet, reset_controlnet, set_controlnet_indexes, load_controlnet
...
class KSamplerRF:
  def __init__(self, model, config, step_function, sigmas, extra_args=None, callback=None, noise_sampler=None):
    ...
    self.pivot_sampling_config = RaveSamplingConfig(
      self.config['latent_shape'],
      self.sigmas,
      self.extra_args,
      self.config['grid_size']
  ...
  def batched_denoise_step(self, x, sigma_index):
    ...
    x_batch = ru.list_to_grid(x[pivotal_idx], grid_size).to(self.config['device'])
    self.sampling_config.prepare_run(sigma_index, pivotal_idx, grid_size)
    self.denoise_step(x_batch, sigma_index, True)
    ...
    for i, b in enumerate(range(0, len(x), batch_size)):
        ...
        self.sampling_config.prepare_run(sigma_index, batch_indexes, grid_size)
        ...
        batch = self.denoise_step(
            x_batch, sigma_index, False)
        batch = ru.shuffle_tensor_batch_other(ru.grid_to_list(batch, grid_size), indexes, list(range(length)))
        denoised_latents.append(batch)
  ...
  def _before_sampling(self):
    ...
    for cond in self.extra_args['cond']:
      prepare_controlnet(cond.get('control', None), 3)
    for cond in self.extra_args['uncond']:
      prepare_controlnet(cond.get('control', None), 3)
...
class KSamplerRFNode():
  ...
  def sample(self,
    ...
    for cond in positive:
      load_controlnet(cond[1].get('control', None), 3)
