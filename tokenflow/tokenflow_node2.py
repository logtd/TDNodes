...
class KSamplerTF:
  ...
  def batched_denoise_step(self, source_x, x, sigma_index):
    n_frames = len(x)
    batch_size = min(
    ...
    self.pivot_sampling_config.reset()
    ...
    self.extra_args['apply_options']['src_latent'] = source_x[pivotal_idx].to(self.config['device'])
    ...
    for i, b in enumerate(range(0, len(x), batch_size)):
        ...
        self.extra_args['apply_options']['src_latent'] = source_x[batch_indexes].to(self.config['device'])
  ...
  def prepare_noisy_latent(self):
    noisy_latent = self.config['latent_inversion']
    noisy_latent = self.model.process_latent_in(noisy_latent)
    return noisy_latent
  
  def get_ddim_eps(self, src_latent, inversion_latent, sigma):
    alpha_prod_T = get_alphacumprod(sigma)
    mu_T, sigma_T = alpha_prod_T ** 0.5, (1 - alpha_prod_T) ** 0.5
    eps = (inversion_latent - mu_T * src_latent) / sigma_T
    return eps
  ...
  def sample_loop(self, src_latent):
    noisy_latent = self.prepare_noisy_latent()
    eps = self.get_ddim_eps(src_latent, noisy_latent, self.sigmas[0].to(src_latent.device))
    x = self.add_noise(src_latent, eps, self.sigmas[0].to(src_latent.device))
    for i in tqdm(range(len(self.sigmas)-1), desc="Sampling"):
      x = self.batched_denoise_step(src_latent, x, i)
      self.callback(
...
class KSamplerTFNode():
  ...
  def sample(self,
    ...
    latent_image = latent_image['samples']  # switched for now
